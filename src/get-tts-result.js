 ...
 * @param {Integer?} option.ModelType -  模型类型，1-默认模型
 * @param {Float?} option.Volume - 音量大小，范围：[0，10]，分别对应11个等级的音量，默认为0，代表正常音量。没有静音选项。输入除以上整数之外的其他参数不生效，按默认值处理。
 * @param {Float?} option.Speed - 语速，范围：[-2，2]，分别对应不同语速
 * @param {Integer?} option.ProjectId - 项目id，用户自定义，默认为0
 * @param {Integer?} option.VoiceType	- 音色
 * @param {Integer?} option.PrimaryLanguage - 主语言类型 1：中文（默认） 2：英文
 * @param {Integer?} option.SampleRate - 音频采样率：16000：16k（默认）8000：8k
 * @param {string?} option.Codec - 返回音频格式，可取值：wav（默认），mp3
